...
def construir_matriz_tf_idf(indice_invertido, documentos):
    ...
    for key in indice_invertido:
        ...
        for doc_idx in indice_invertido[key]:
            M[row, doc_idx] += 1
        
        row += 1

    N = len(documentos)
    df = np.sum(M > 0, axis = 1) # esse M > 0 torna os valores da matriz em True (valor>0) ou False (valor<=0)
    idf = np.log(N / df)
    ...
    idf = idf.reshape(-1, 1) (torna o array de idf em uma matrix)
    M = idf * M
    
    ou...
    
    for row_data in M:
        term_in_docs = [count for count in row_data if count > 0]
        idf = np.log(N / len(term_in_docs))
        row_data *= idf
    '''
    return (M, ids_termos)
...
def consultar(termos_consulta, M, ids_termos, documentos):
    ...
    t_ids = [ids_termos[t] for t in termos_consulta]
    q[t_ids] = 1
    ...
    for j in range(M.shape[1]):
        m =  M[:, j]
        m_norm = np.sqrt(np.square(m).sum())
        p = np.dot(q.T, m)[0]
        cs = p / (q_norm * m_norm)
        results.append(cs)

    for doc_id in np.argsort(results)[::-1]:
